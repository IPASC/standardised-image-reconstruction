...
def get_first_value(vec, val):
    for i in range(0, len(vec)):
        if vec[i] < val:
            return i
    # fallback: return first entry from given vector
    return 0


def compute_fwhm(reconstructed_image,
                 spacing,
                 non_genativity_method="log"):
...
class GeneralisedSignalToNoiseRatio(NoReferenceMeasure):
    ...
    def compute_measure(self, reconstructed_image, signal_roi=None, noise_roi=None):
        """
        ...
        return self.compute_gCNR(reconstructed_image, signal_roi, noise_roi)

    def compute_gCNR(self, reconstructed_image, signal_roi=None, noise_roi=None):

        if signal_roi is None or noise_roi is None:
            raise AssertionError("The ROIs must not be set for gCNR.")
